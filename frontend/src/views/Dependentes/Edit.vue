    ...
    <div class="row">
      <div class="col-md-8 mx-auto">
        <div class="card shadow">
          <div class="card-body">
            <h2 class="mb-4">Editar Dependente</h2>
...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      // Carregar lista de carteiristas
      await store.dispatch('carteiristas/fetchCarteiristas')
      
      const id = route.params.id
      const resp = await store.dispatch('dependentes/fetchDependente', id)
      
      if (resp) {
        form.nome = resp.nome || ''
        form.cpf = resp.cpf || ''
        form.idade = resp.idade || null
        form.carteirista_uuid = resp.carteirista_uuid || ''
        form.ativo = resp.ativo !== undefined ? resp.ativo : true
        
        // Format CPF if it exists
        if (form.cpf) {
          const cpfNumbers = form.cpf.replace(/\D/g, '').substring(0, 11)
          if (cpfNumbers.length > 0) {
            if (cpfNumbers.length <= 3) {
              form.cpf = cpfNumbers
            } else if (cpfNumbers.length <= 6) {
              form.cpf = `${cpfNumbers.substring(0, 3)}.${cpfNumbers.substring(3)}`
            } else if (cpfNumbers.length <= 9) {
              form.cpf = `${cpfNumbers.substring(0, 3)}.${cpfNumbers.substring(3, 6)}.${cpfNumbers.substring(6)}`
            } else {
              form.cpf = `${cpfNumbers.substring(0, 3)}.${cpfNumbers.substring(3, 6)}.${cpfNumbers.substring(6, 9)}-${cpfNumbers.substring(9)}`
            }
          }
        }
      }
    ...
    const onSubmit = async () => {
      ...
      loading.value = true
      const id = route.params.id
      const res = await store.dispatch('dependentes/updateDependente', { id, data: form })
      loading.value = false
      
      if (res && res.success === false) {
        errorMessage.value = res.message || 'Erro ao atualizar dependente'
      } else {
